...
type Metrics = {
  ...
  visible_people_median?: number;
  visible_people_min?: number;
  visible_people_max?: number;
};
...
export function ResultsWorkspace({ project, tracks, roster, urls }: { project: Project; tracks: Track[]; roster: RosterPlayer[]; urls: ResultUrls }) {
  ...
  const currentFrame = Math.round(time * 15);
  const visibleNow = useMemo(() => trackData.filter((track) => (track.detections ?? track.trajectory).some((point) => point.frame === currentFrame)).length, [currentFrame, trackData]);
  ...
  return (
    <main className="page-shell"><Topbar /><div className="container content">
      <div className="page-heading"><div><span className="eyebrow">INTERACTIVE RESULT</span><h1>{project.title}</h1><p>{trackData.length} persistent tracks · {visibleNow} visible at current frame · click a box to inspect one pixel mask</p></div><div className="nav-actions"><span className="status-pill completed"><ShieldCheck size={13} />A40 COMPLETE</span><a className="button button-primary" href={urls.foreground} download><Download size={16} /> DOWNLOAD FOREGROUND MP4</a></div></div>
      ...
      <div className="result-grid">
        <div className="form-stack">
          ...
          <section className="panel"><div className="panel-title"><div><h2>Pitch activity heatmap</h2><p className="micro">Smoothed field positions · {selectedId === null ? "all tracks" : `track ${selectedId}`}</p></div><span>{selected?.metrics.metric_calibration_available === false ? "METRIC CALIBRATION UNAVAILABLE" : "105M × 68M"}</span></div><PitchMap tracks={trackData} selectedId={selectedId} /></section>
        </div>
        <aside className="form-stack">
          <section className="panel player-card">
            ...
            </> : <div className="empty-state"><Eye size={28} /><h2>Select a player</h2><p>All tracks stay lightweight until you click one. Its SAM mask and full trajectory then load on demand.</p><span className="micro">Use the native video controls to play, pause, or seek. The trajectory toggle shows a subtle path for every track until one is selected.</span></div>}
          </section>
